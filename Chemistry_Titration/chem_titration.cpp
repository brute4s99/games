...
void main()
{
    ...
    for (i = 0; i < v2; i++) {
        ...
        if (i != 0) {
            ...
            sprintf(val, "Normality of KMnO4 = ??? N");
...
void fall()
{
    ...
    for (i = 5; i < fps - 5; i++) {
        ...
        ellipse(mid, 357, 0, 360, 16, 4);
        ellipse(mid, 356, 0, 360, 10, 4);
        if (getpixel(mid + 5, y + 5) == LIGHTMAGENTA || getpixel(mid + 5, y + 4) == LIGHTMAGENTA)
            break;
    }
    setfillstyle(1, LIGHTMAGENTA);
    f_lvl++;
...
void flasklvl()
{
    ...
    if (y >= 430) {
        ...
        for (i = mid; i > mid - 40; i--)
            if (getpixel(i, y) == WHITE) {
                x1 = i;
                break;
            }
        x2 = mid + mid - x1;
